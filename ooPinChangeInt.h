...
class IntCallBackInterface
{
   public:

     CallBackInterface() {};

     virtual void cbmethod(bool pinState) {
     };
...
public:
	...
	static		int8_t attachInterrupt(uint8_t pin, IntCallBackInterface* cbIface, int mode);
...
protected:
	...
	public:
		PCintPin() :
		mode(0) {}
		IntCallBackInterface* 	pinCallBack;
		uint8_t 	mode;
		uint8_t		mask;
		#ifndef NO_PIN_STATE
		uint8_t		state;
		...
		while (p) {
			if (p->mask & changedPins) { // a changed bit
				...
				// the bit is currently high, or if mode is FALLING and bit is low.
					uint8_t pinState = PCintPort::curr & p->mask ? HIGH : LOW;
					#ifndef NO_PIN_STATE
					p->state=pinState
					#endif
					(*(p->pinCallBack)).cbmethod(pinState);
				//}
			}
